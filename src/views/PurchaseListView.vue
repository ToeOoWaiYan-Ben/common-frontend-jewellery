<template>
  <div class="page">
    ...
    <div class="header">
      ...
      <div class="actions">
        <input v-model="search" class="input" placeholder="Search invoice / customer / status" />
        <button class="btn primary" @click="createNew">+ New Purchase</button>
      </div>
    </div>
        ...
        <table class="table">
          ...
          <tbody>
            <tr v-for="p in filtered" :key="p.id">
              ...
              <td class="right">
                <button class="btn small" @click="open(p.id)">Open</button>
                <button class="btn small danger" @click="del(p.id)">Delete</button>
              </td>
            </tr>

            <tr v-if="!filtered.length">
              <td colspan="6" class="empty">No purchases found.</td>
            </tr>
          </tbody>
        </table>

        <div class="note">Frontend only — data saved in <b>localStorage</b>.</div>
      </div>
    </div>
  </div>
</template>
...
<script setup lang="ts">
  ...
  const filtered = computed(() => {
    ...
    return store.purchases.filter((p) => {
      const text = [p.invoiceNo, p.date, p.status, p.customer?.name, p.customer?.phone]
        .join(' ')
        .toLowerCase()
