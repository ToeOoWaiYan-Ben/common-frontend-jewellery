<template>
  <section>
    ...
    <div class="card" style="margin-bottom:14px;">
      <div v-if="error" class="users-empty users-empty--error" style="margin-bottom:10px;">{{ error }}</div>

      <div class="category-form">
        <div class="category-form__row">
          <label class="category-form__label">Name *</label>
          <input v-model.trim="form.name" class="category-form__input" placeholder="e.g. John Seller" />
        </div>

        <div class="category-form__row">
          <label class="category-form__label">Phone</label>
          <input v-model.trim="form.phone" class="category-form__input" placeholder="e.g. 09xxxxxxx" />
        </div>

        <div class="category-form__row">
          <label class="category-form__label">Email</label>
          <input v-model.trim="form.email" class="category-form__input" placeholder="e.g. john@gmail.com" />
        </div>

        <div class="category-form__row">
          <label class="category-form__label">Address</label>
          <input v-model.trim="form.address" class="category-form__input" placeholder="e.g. Bangkok" />
        </div>

        <div style="display:flex; gap:10px; justify-content:flex-end; margin-top:12px;">
          <button class="btn-secondary" type="button" @click="reset" :disabled="store.loading">Reset</button>
          <button class="btn-primary" type="button" @click="submit" :disabled="store.loading || !form.name">
    ...
    <div class="card">
      <div class="table-wrapper">
        <table class="admin-table">
          ...
          <tbody>
            <tr v-if="store.items.length === 0"><td colspan="6">No data.</td></tr>
            <tr v-for="s in store.items" :key="s.id" :class="{ 'row--editing': s.id === editingId }">
              <td>{{ s.id }}</td>
              <td>{{ s.name }}</td>
              <td>{{ s.phone ?? '-' }}</td>
              <td>{{ s.email ?? '-' }}</td>
              ...
              <td style="text-align:center;">
                ...
                <button class="pagination-btn" style="border-color:#fecdd3; background:#fff1f2; color:#9f1239;"
                        @click="del(s)">
                  Delete
                </button>
              </td>
...
<style scoped>
.row--editing td { background: #e6ffed; }
</style>
