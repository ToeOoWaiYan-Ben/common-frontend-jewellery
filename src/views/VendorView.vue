<template>
  ...
  <div class="search-row">
    ...
    />
    <button class="btn-secondary" type="button" @click="handleSearchInvoice">
      Search
    </button>
  </div>
  ...
  >
    Clear Invoice
      ...
      <div
  v-if="selectedInvoice && Array.isArray(selectedInvoice.items) && selectedInvoice.items.length > 0"
...
>
  ...
  <div class="invoice-products">
    ...
    >
      ...
      <button class="btn-mini" type="button" @click="addSellBackItem(item)">
        add
      </button>
    </div>
  </div>
      ...
      <div class="vendor-form__actions">
        ...
        <button class="btn-primary" type="button" @click="handleSubmitForm" :disabled="isSubmitting">
...
const filteredVendors = computed(() => {
  ...
  return vendors.value.filter((v: any) => {
    return (
      String(v.desc ?? '').toLowerCase().includes(term) ||
